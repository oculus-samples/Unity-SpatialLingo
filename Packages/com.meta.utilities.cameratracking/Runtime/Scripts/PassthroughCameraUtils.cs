...
namespace Meta.Utilities.CameraTracking
{
    public static class PassthroughCameraUtils
    {
        ...
        public static bool IsSupported
        {
            get
            {
                if (!s_isSupported.HasValue)
                {
                    ...
                    return (headset is OVRPlugin.SystemHeadset.Meta_Quest_3 or
                            OVRPlugin.SystemHeadset.Meta_Quest_3S or
                            OVRPlugin.SystemHeadset.Meta_Link_Quest_3 or
                            OVRPlugin.SystemHeadset.Meta_Link_Quest_3S) &&
                        HorizonOSVersion is null or 0 or >= MINSUPPORTOSVERSION;
                }
        ...
        /// </summary>
        /// <param name="cameraEye">The passthrough camera</param>
        private static Dictionary<PassthroughCameraEye, PassthroughCameraIntrinsics> s_cachedIntrinsics = new();
        public static PassthroughCameraIntrinsics GetCameraIntrinsics(PassthroughCameraEye cameraEye)
        {
            if (s_cachedIntrinsics.ContainsKey(cameraEye))
            {
                return s_cachedIntrinsics[cameraEye];
            }
            ...
            var intrisic = new PassthroughCameraIntrinsics
            {
...
#else
            var intrisic = default(PassthroughCameraIntrinsics);
#endif
            s_cachedIntrinsics[cameraEye] = intrisic;
            return intrisic;
        }
...
#else
            CameraEyeToCameraIdMap[PassthroughCameraEye.Left] = ("", 0);
            CameraEyeToCameraIdMap[PassthroughCameraEye.Right] = ("", 0);
#endif
